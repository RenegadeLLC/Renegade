...
$pageHTML .= '<div class="outer-wrapper" style="background:#E6E7E8;">';
...
// GET GRIDS
	...
	while ( have_rows('grid') ) : the_row();	
		...
		$custom_grid_class = get_sub_field('custom_grid_class');
		...
		if($grid_num == 0):
			...
			if($grid_width == 'Yes'):
				$pageHTML .= '<div class="wrapper home-top">';
			endif;
		...
		$pageHTML .= '<div class="grid ' . $grid_class; 
			if($custom_grid_class):
				$pageHTML .= ' ' . $custom_grid_class;
			endif;
		$pageHTML .= '"';
...
//GET CUSTOM CLASS
		...
		if($section_background_color || $section_background_image || $text_color || $module_height):
			...
			$pageHTML .= ' style="';
				...
				if($module_height):
					$pageHTML .= 'min-height:' . $module_height . '; ';
				endif;
				...
				if($section_background_image && $section_background_image !=''):
					...
					$pageHTML .= ' background-image:url(' . $section_background_image . '); background-repeat:no-repeat;';
				endif;
		/*
				...
				*/
				...
				if($section_background_image_position):
					$pageHTML .= ' background-position:' . $section_background_image_position . ';';
				endif;
				
				if($section_background_image_size):
					$pageHTML .= ' background-size:' . $section_background_image_size . ';';
				endif;

				if($module_height && $content_type != 'Text Highlight'):
					$pageHTML .= ' min-height:' . $module_height . 'px;';
				elseif($module_height):
		...
		switch($content_type){
			...
			case 'Image':
				$pageHTML .= '<img src="' . $image . '" class="lazy">';
			break;
			...
			switch($post_type){
					case 'post':
						//$pageHTML .= '<h2>Saw a Good Idea</h2>';
						break;
					case 'rss':
