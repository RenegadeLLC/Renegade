...
function get_rss_func($atts, $content = null){
	...
	/*	foreach($post_categories as $category):
		...
		$rssHtml .= '><h2><a class="rss" href="' . esc_url( $item->get_permalink() ) . '">' . esc_html( $item->get_title() ) . '</a></h2>';
		$rssHtml .= '<div class="rss-date">';
		$rssHtml .= $item->get_date('j F Y | g:i a') . '</div>' ;
		$rssHtml .= $item->get_description();
		$rssHtml .= '<div><div class="bt-black"><a class="rss" href="' . esc_url( $item->get_permalink() ) . '" target="_blank" style="color:#fff !important; opacity:1 !important;">Read It</a></div></div>';
		$rssHtml .= '</li>';
