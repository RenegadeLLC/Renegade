<?php
if (!class_exists("wpdreamsCustomTaxonomyTermSel")) {
    /**
     ...
     */
    class wpdreamsCustomTaxonomyTermSel extends wpdreamsType {
        function getType() {
            parent::getType();
            $this->processData();
            $this->types = $this->getAllTerms();
            echo "
      <div class='wpdreamsCustomTaxonomyTermSel' id='wpdreamsCustomTaxonomyTermSel-" . self::$_instancenumber . "'>
        <fieldset>                               
          ...
          <legend>" . $this->label . "</legend>";
            ...
            if ( !empty($this->selected) ) {
                foreach ($this->selected as $k => $v) {
                    ...
                    $checked = in_array($term->term_id, $this->not_checked) ? "" : " checked='checked'";
                    echo '<li class="ui-state-default" term_id="' . $term->term_id . '" taxonomy="' . $term->taxonomy . '">' . $term->name . '
                            <input style="float:right;" type="checkbox" value="' . $term->term_id . '" '.$checked.' />
                         </li>';
                }
                $this->data["value"] = implode("|", $this->verified_checked) . "|-|" . implode("|", $this->not_checked);
            }
            echo "</ul></div>";
            echo "
         <input isparam=1 type='hidden' value='" . $this->data["value"] . "' name='" . $this->name . "'>
         <input type='hidden' value='wpdreamsCustomTaxonomyTermSel' name='classname-" . $this->name . "'>";
            echo "
        </fieldset>
      </div>";
        }
        ...
        function printTermsRecursive ($terms, $level = 0) {
            foreach ($terms as $term) {
                if ($this->selected == null || !wd_in_array_r($term->term_id, $this->selected))
                    echo '<li class="ui-state-default termlevel-'.$level.'" term_id="' . $term->term_id . '" taxonomy="' . $term->taxonomy . '">' . $term->name . '
                    <input style="float:right;" type="checkbox" value="' . $term->term_id . '" checked="checked"/>
                    </li>';
        ...
        function processData() {
            ...
            if ($this->v != "") {
                ...
                if (!empty($_arr[1]))
                    $this->not_checked = explode("|", $_arr[1]);
            }

            $this->_selected = array(
                "selected" => $selected,
                "not_checked" => $this->not_checked
            );
        }
