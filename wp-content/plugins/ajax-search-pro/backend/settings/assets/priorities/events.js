...
// --------------------------------------- INIT VARIABLES --------------------------------------------------------------
    ...
    var groupsObject = JSON.parse(Base64.decode($('#priority_groups').val()));
...
// -------------------------------------- GROUP LISTING & BUTTONS ------------------------------------------------------
    ...
    $('#pg_container').on('click', '.pg_rg_edit', function(){
        ...
        mw.options({
            'type': 'info',
            'header': 'Edit priority group: ' + openedRuleGroup.name,
            'leaveContent': true,
            'buttons': {
                'okay': {
                    'text': 'Save!',
                    ...
                    'click': function(e, button){
                        ...
                        listController.printGroups();
                        $('#priority_groups').val(Base64.encode(JSON.stringify(groupsObject)));
                    }
                },
                'cancel': {
                    'text': 'Cancel',
    ...
    $('#pg_container').on('click', '.pg_rg_delete', function(){
        ...
        if ( typeof groupsObject[id] == 'undefined' )
            return false;
        var sure = confirm('Are you sure you want to delete "'+groupsObject[id].name+'" ?');
        if ( sure ) {
    ...
    $('#pg_remove_all').on('click', function(e){
        e.preventDefault();
        if ( confirm('Are you sure you want to delete all groups? This is not reversible!') ) {
...
// -------------------------------------- RULE EDITOR OTHER INTERACTIONS -----------------------------------------------
    ...
    $('#asp_pg_editor .wd_taxterm_search_res').on('click', 'li:not(.wtsr_selected)', function(e){
        if ( $('#pg_selected_tax_terms li').length < 11 ) {
            ...
            alert('Only 10 categories are allowed per rule!');
        }
    });
